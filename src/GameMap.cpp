...
bool GameMap::LoadFromFile(const std::string& filename)
{
    ...
    gSystem.LogMessage(eLogMessage_Info, "Loading map data '%s'", filename.c_str());

    std::ifstream file;
    if (!gSystem.mFiles.OpenBinaryFile(filename, file))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot open map data file");
        return false;
    }
    ...
    {
        gSystem.LogMessage(eLogMessage_Warning, "Invalid map file header");
        return false;
    }

    if (!ReadCompressedMapData(file, header.column_size, header.block_size))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot read compressed map data");
        return false;
    }

    if (!ReadStartupObjects(file, header.object_pos_size))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot read startup objects data");
        return false;
    }

    if (!ReadRoutes(file, header.route_size))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot read routes data");
        return false;
    }

    if (!ReadServiceBaseLocations(file))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot read service base locations data");
        return false;
    }

    if (!ReadNavData(file, header.nav_data_size))
    {
        gSystem.LogMessage(eLogMessage_Warning, "Cannot read nav data");
        return false;
    }
...
bool GameMap::ReadCompressedMapData(std::istream& file, int columnLength, int blocksLength)
{
    // reading base data
...
const MapBlockInfo* GameMap::GetBlockInfo(int coordx, int coordz, int layer) const
{
...
const MapBlockInfo* GameMap::GetNeighbourBlock(int coordx, int coordz, int layer, eMapDirection2D dir) const
{
...
float GameMap::GetWaterLevelAtPosition2(const glm::vec2& position) const
{
...
const DistrictInfo* GameMap::GetDistrictAtPosition2(const glm::vec2& position) const
{
    glm::ivec2 blockPosition = Convert::MetersToMapUnits(position);
...
const DistrictInfo* GameMap::GetDistrict(int coordx, int coordy) const
{
...
const DistrictInfo* GameMap::GetDistrictByIndex(int districtIndex) const
{
...
float GameMap::GetHeightAtPosition(const glm::vec3& position, bool excludeWater) const
{
...
bool GameMap::TraceSegment2D(const glm::vec2& origin, const glm::vec2& destination, float height, glm::vec2& outPoint)
{
...
bool GameMap::ReadStartupObjects(std::istream& file, int dataSize)
{
...
bool GameMap::ReadRoutes(std::istream& file, int dataSize)
{
    file.seekg(dataSize, std::ios::cur);
    return true;
}

bool GameMap::ReadServiceBaseLocations(std::ifstream& file)
{
    struct LocationData
    {
        unsigned char x;
...
bool GameMap::ReadNavData(std::ifstream& file, int dataSize)
{
...
std::string GameMap::GetStyleFileName(int styleNumber) const
{
...
std::string GameMap::GetStyleFileName() const
{
    return GetStyleFileName(mStyleFileNumber);
}
